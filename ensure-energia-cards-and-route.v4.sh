#!/bin/bash
# ensure-energia-cards-and-route.v4.sh
# -------------------------------------------------------------------
# Implementa las 4 cards de ENERGÍA (AVR/PLANTAS/CORPOELEC/INVERSOR)
# y asegura que Dashboard.jsx RENDERICE esa vista cuando el hash
# sea #/energia (o el route.name sea 'energia').
#
# Hace TODO:
#   1) Crea/actualiza helpers, overview, detail, css y Energia.jsx.
#   2) Sanea imports de Energia en Dashboard.jsx (sin duplicados).
#   3) Inyecta un "gate" robusto en Dashboard.jsx para #/energia.
#   4) Ajusta cualquier link a Energía -> href="#/energia".
#   5) Limpia caché de Vite y reinicia dev server.
#
# Uso:
#   chmod +x ./ensure-energia-cards-and-route.v4.sh
#   ./ensure-energia-cards-and-route.v4.sh
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 1) Helpers (categoriza y calcula métricas/SLA)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 2) Overview (4 cards) con estilo tipo instancias
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 3) Detalle por categoría (lista + gráficas si MultiServiceView existe)
# -------------------------------------------------------------------
cat > "$DETAIL_FILE" <<'EOF'
...
# -------------------------------------------------------------------
cat > "$CSS_FILE" <<'EOF'
/* src/views/energia-cards-v4.css */
...
EOF
ok "Energia.jsx actualizado: $ENERGIA_FILE"

# -------------------------------------------------------------------
# 6) Saneado de imports y gate en Dashboard.jsx
# -------------------------------------------------------------------
# 6a) eliminar cualquier import desde Energia.default.jsx
sed -E -i '/^[[:space:]]*import[[:space:]]+Energia[[:space:]]+from[[:space:]]*["'\''"]\.[\.\/]*Energia\.default\.jsx["'\''"][[:space:]]*;[[:space:]]*$/d' "$DASHBOARD_FILE"

# 6b) eliminar imports de Energia desde Energia.jsx (para reinsertar uno único)
...
# 6d) inyectar gate robusto una sola vez
if ! grep -q "__ENERGIA_GATE_V4__" "$DASHBOARD_FILE"; then
  # localizar declaración de Dashboard
  LINE_DECL="$(awk '
  ...
  if [ -n "$LINE_DECL" ]; then
    ...
    awk -v ins="$BODY_START" '
      NR==ins {
        print $0
        print "  // __ENERGIA_GATE_V4__"
        print "  try {"
        print "    const h = (typeof window !== \"undefined\" ? (window.location.hash || \"\") : \"\").toLowerCase();"
        print "    const routeName = (typeof route !== \"undefined\" && route && route.name ? String(route.name).toLowerCase() : \"\");"
        print "    if (h.startsWith(\"#/energia\") || h === \"#energia\" || h.includes(\"energia\") || routeName === \"energia\") {"
        print "      return <Energia monitorsAll={(typeof monitors !== \"undefined\" ? monitors : (typeof props !== \"undefined\" ? (props.monitorsAll || props.monitors || []) : []))} />;"
        print "    }"
        print "  } catch (e) {}"
        next
      }
    ...
    ok "Gate insertado en Dashboard.jsx"
  else
    warn "No pude detectar la declaración del componente Dashboard; omito gate."
  fi
else
  ok "Gate ya estaba presente (no se duplica)"
fi

# 6e) apuntar links visibles a #/energia
sed -E -i "s#(href=)[\"']([^\"']*([Ee]nergia|[Ee]nerg#g" "$DASHBOARD_FILE" || true
...
echo
ok "Listo. Ve a #/energia (o haz clic en 'Energía') para ver las cards. Backups: Dashboard.jsx.backup.${TS} (y Energia.jsx.backup.${TS} si existía)"
