...
function ModalDetalle({ isOpen, onClose, titulo, monitores, tipoFiltro }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ margin: '0 0 20px 0', color: '#1f2937' }}>{titulo}</h2>
...
function TarjetaTipo({ tipo, monitores, datos }) {
  ...
  const [filtroModal, setFiltroModal] = useState(null); // 'up', 'down', 'issues'
  ...
  const handleVerDetalle = (tipoFiltro) => {
    setFiltroModal(tipoFiltro);
    setModalOpen(true);
  };
  ...
  const monitoresFiltrados = useMemo(() => {
    ...
    return monitores.filter(m => {
      ...
      if (filtroModal === 'up') return status === 1;
      if (filtroModal === 'down') return status === 0 || rt === -1;
      if (filtroModal === 'issues') return status !== 1 && status !== 0 && rt !== -1;
      return true;
    });
  }, [monitores, filtroModal]);
  ...
  return (
    <>
      <div className="k-card" style={{
        ...
        {/* Métricas principales con botones clickeables */}
        <div style={{ padding: '20px' }}>
          <div style={{
            ...
            <button
              onClick={() => handleVerDetalle('up')}
              style={{
                background: '#f9fafb',
                padding: '12px',
                borderRadius: '8px',
                textAlign: 'center',
                border: up > 0 ? '2px solid #16a34a' : '1px solid #e5e7eb',
                cursor: up > 0 ? 'pointer' : 'default',
                transition: 'all 0.2s ease',
                opacity: up > 0 ? 1 : 0.5
              }}
              onMouseEnter={(e) => {
                if (up > 0) e.currentTarget.style.background = '#f3f4f6';
              }}
              onMouseLeave={(e) => {
                if (up > 0) e.currentTarget.style.background = '#f9fafb';
              }}
            >
              ...
              {up > 0 && <div style={{ fontSize: '0.7rem', color: '#6b7280', marginTop: '4px' }}>Click para ver</div>}
            </button>
            
            <button
              onClick={() => handleVerDetalle('down')}
              style={{
                background: '#f9fafb',
                padding: '12px',
                borderRadius: '8px',
                textAlign: 'center',
                border: down > 0 ? '2px solid #dc2626' : '1px solid #e5e7eb',
                cursor: down > 0 ? 'pointer' : 'default',
                transition: 'all 0.2s ease',
                opacity: down > 0 ? 1 : 0.5
              }}
              onMouseEnter={(e) => {
                if (down > 0) e.currentTarget.style.background = '#f3f4f6';
              }}
              onMouseLeave={(e) => {
                if (down > 0) e.currentTarget.style.background = '#f9fafb';
              }}
            >
              ...
              {down > 0 && <div style={{ fontSize: '0.7rem', color: '#6b7280', marginTop: '4px' }}>Click para ver</div>}
            </button>
            
            <button
              onClick={() => handleVerDetalle('issues')}
              style={{
                background: '#f9fafb',
                padding: '12px',
                borderRadius: '8px',
                textAlign: 'center',
                border: issues > 0 ? '2px solid #f59e0b' : '1px solid #e5e7eb',
                cursor: issues > 0 ? 'pointer' : 'default',
                transition: 'all 0.2s ease',
                opacity: issues > 0 ? 1 : 0.5
              }}
              onMouseEnter={(e) => {
                if (issues > 0) e.currentTarget.style.background = '#f3f4f6';
              }}
              onMouseLeave={(e) => {
                if (issues > 0) e.currentTarget.style.background = '#f9fafb';
              }}
            >
              <div style={{ fontSize: '0.75rem', color: '#6b7280', textTransform: 'uppercase' }}>ISSUES</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '600', color: '#f59e0b' }}>{issues}</div>
              {issues > 0 && <div style={{ fontSize: '0.7rem', color: '#6b7280', marginTop: '4px' }}>Click para ver</div>}
            </button>
          ...
          <div style={{
            ...
            <div style={{
              ...
              height: '100%',
              cursor: up > 0 ? 'pointer' : 'default'
            }} onClick={() => up > 0 && handleVerDetalle('up')} title={`UP: ${up}`} />
            <div style={{
              flex: down,
              background: '#dc2626',
              height: '100%',
              cursor: down > 0 ? 'pointer' : 'default'
            }} onClick={() => down > 0 && handleVerDetalle('down')} title={`DOWN: ${down}`} />
            <div style={{
              flex: issues,
              background: '#f59e0b',
              height: '100%',
              cursor: issues > 0 ? 'pointer' : 'default'
            }} onClick={() => issues > 0 && handleVerDetalle('issues')} title={`Issues: ${issues}`} />
          </div>

          {/* Lista de equipos (primeros 6) */}
          <div>
            ...
            <div style={{
              ...
              {monitores.slice(0, 6).map((monitor, idx) => {
                ...
                return (
                  <div 
                    key={idx} 
                    onClick={() => handleVerDetalle(status === 'up' ? 'up' : 'down')}
                    style={{
                      padding: '12px',
                      background: status === 'up' ? '#f0fdf4' : '#fef2f2',
                      borderRadius: '6px',
            ...
            {monitores.length > 6 && (
              ...
              >
                + {monitores.length - 6} equipos más - Click para ver todos
              </div>
            )}
          </div>
        </div>
      ...
      <ModalDetalle
        ...
        titulo={`${estilo.label} - ${
          filtroModal === 'up' ? 'Equipos UP' :
          filtroModal === 'down' ? 'Equipos DOWN' :
          filtroModal === 'issues' ? 'Equipos con Issues' :
          'Todos los equipos'
        }`}
        monitores={monitoresFiltrados}
        tipoFiltro={filtroModal}
      />
    </>
  );
}
