#!/bin/bash
# crear-plantadetail.sh
# CREA EL COMPONENTE PLANTA DETAIL

echo "====================================================="
echo "📝 CREANDO COMPONENTE PLANTA DETAIL"
echo "====================================================="

FRONTEND_DIR="/home/thunder/kuma-dashboard-clean/kuma-ui"
DETAIL_FILE="$FRONTEND_DIR/src/components/PlantaDetail.jsx"

# ========== 1. CREAR ARCHIVO ==========
echo ""
echo "[1] Creando PlantaDetail.jsx..."
...
export default function PlantaDetail({ planta, onClose, onActualizar }) {
  ...
  const cargarEstadoActual = async () => {
    try {
      ...
      const monitor = data.monitors.find(m => 
        m.instance === 'Energia' && 
        m.info.monitor_name === planta.nombre_monitor
      );
      
      if (monitor) {
        setEstadoActual({
          status: monitor.latest?.status === 1 ? 'UP' : 'DOWN',
          responseTime: monitor.latest?.responseTime || 0,
          lastCheck: monitor.latest?.timestamp || Date.now()
        });
      }
    } catch (error) {
  ...
  const cargarConsumo = () => {
    ...
    if (saved) {
      ...
      setConsumo(data[planta.nombre_monitor] || { sesionActual: 0, historico: 0 });
    }
  };
  ...
  const resetearContador = () => {
    if (window.confirm('¿Resetear el contador de consumo de esta planta?')) {
      ...
      if (saved) {
        const data = JSON.parse(saved);
        delete data[planta.nombre_monitor];
        localStorage.setItem('consumo_plantas', JSON.stringify(data));
        setConsumo({ sesionActual: 0, historico: 0 });
      }
    }
  };
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>
              {estadoActual?.responseTime?.toFixed(2) || '0'} ms
            </div>
          </div>
...
echo "✅ PlantaDetail.jsx creado"

# ========== 2. HACER BUILD NUEVAMENTE ==========
echo ""
echo "[2] Intentando build nuevamente..."
cd "$FRONTEND_DIR"
npm run build

echo ""
echo "====================================================="
echo "✅✅ COMPONENTE PLANTA DETAIL CREADO ✅✅"
echo "====================================================="
echo ""
