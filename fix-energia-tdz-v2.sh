#!/bin/bash
# fix-energia-tdz-v2.sh
# -------------------------------------------------------------------
# Corrige:
#   - "Cannot access 'monitors' before initialization" (TDZ)
#   - Errores previos de regex/gate e inserciones repetidas
#
# Qué hace:
#   1) Hace backup de src/views/Dashboard.jsx
#   2) Deja UN SOLO import:  import Energia from "./Energia.jsx";
#      (elimina import nombrado y cualquier import desde Energia.default.jsx)
#   3) Elimina gates/regex previos (__ENERGIA_GATE_V*/isEnergiaRoute viejos)
#   4) Inserta utilidad isEnergiaRoute() SIN regex y un EARLY RETURN que
#      NO toca la variable local `monitors` (usa solo props/arguments[0])
#   5) Reinicia Vite limpiando la caché
#
# Uso:
#   chmod +x ./fix-energia-tdz-v2.sh
#   ./fix-energia-tdz-v2.sh
# -------------------------------------------------------------------
...
#    - quita el shim default y duplicados/nombrados
...
const __ENERGIA_GATE_V9__ = true;\\n\
const isEnergiaRoute = () => {\\n\
  try {\\n\
    ...
    // Quita \"#/\" y separa segmentos: ['energia'] o ['energia','slug']\\n\
  ...
  ok "Insertada utilidad isEnergiaRoute (v9, sin regex)"
else
  warn "No se detectaron imports; no se pudo insertar la utilidad tras imports."
fi

# 5) Inserta EARLY RETURN dentro del componente Dashboard SIN tocar `monitors`
#    (usa solo props/arguments[0]) — evita TDZ
...
awk '
...

  {
    ...
    if (!inserted && startDecl>0 && NR>=startDecl && index($0,"{")) {
      print "  // EARLY RETURN para ruta de Energía v9 (evita TDZ — no usa variable local `monitors`)"
...
ok "Early return v9 insertado (sin TDZ)"

# 6) Reiniciar Vite (limpia caché + arranca dev)
...
echo
ok "Arreglo aplicado. Abre #/energia para que Dashboard renderice la vista de Energía."
