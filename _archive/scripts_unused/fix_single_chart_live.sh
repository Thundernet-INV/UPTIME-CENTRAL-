...
echo "== 1) Quitar fallbacks en App.jsx (import/render de OverlayChart/ChartFallback) =="
# Borrar imports de OverlayChart / ChartFallback si existieran
...
# Borrar render de OverlayChart/ChartFallback si existieran
sed -i '/<OverlayChart .*\/>/d' "$APP" || true
sed -i '/<ChartFallback .*\/>/d' "$APP" || true

echo "== 2) Reescribir InstanceDetail.jsx SIN fallbacks y con datos en vivo (arrays reales) =="
cat > "$ID" <<'JSX'
...
echo "✓ Hecho: una sola gráfica (nativa) y datos en vivo (se actualiza cada 5s)."
