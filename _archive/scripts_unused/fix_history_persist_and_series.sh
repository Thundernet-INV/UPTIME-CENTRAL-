...
echo "== 1) historyDB.js (IndexedDB con utilitarios completos) =="
cat > "$HDB" <<'JS'
...
echo "== 2) historyEngine.js (persistencia + compat RESTAURADA) =="
cat > "$HIST" <<'JS'
import DB from './historyDB';

/**
 * Capa de historial con IndexedDB + compat para UI legada.
 * - addSnapshot(monitors): persiste y poda 7 días
...
echo "✓ Listo: persistencia reactivada y funciones de series expuestas."
